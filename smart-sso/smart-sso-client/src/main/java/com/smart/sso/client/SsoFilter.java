...
public class SsoFilter extends ClientFilter {
	...
	public boolean isAccessAllowed(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (token == null) {
			...
			if (token != null) {
				logger.debug("Header_token="+token);
			...
			logger.debug("Local_token="+token);
			return true;
		}
	...
	private void invokeAuthInfoInSession(HttpServletRequest request, String token) throws IOException {
		...
		if (rpcUser != null) {
			Integer userId=rpcUser.getUserId();
			SessionUser sessionUser=new SessionUser(token, userId,rpcUser.getAccount());
			sessionUser.setRoleBusiCode(authenticationRpcService.findRoleBusiCode(userId));
			sessionUser.setManageUserIds(authenticationRpcService.findUserByChargeUserId(userId));
	...
	private void redirectLogin(HttpServletRequest request, HttpServletResponse response) throws IOException {
		if (isAjaxRequest(request)) {
			...
			responseJson(response, SsoResultCode.SSO_TOKEN_ERROR, "未登录或已超时",ssoLoginUrl);
		}
		else {
	...
	private String getRemoveTokenBackUrl(HttpServletRequest request) {
		...
		int index=backUrl.indexOf(SSO_TOKEN_NAME);
		if(index>2) {
			return backUrl.substring(0, index - 1);
		}
		return backUrl;
	}
